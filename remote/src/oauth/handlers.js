...
import { createDb, tOAuthClients, tOAuthAuthorizationCodes, tOAuthAccessTokens, tOAuthRefreshTokens, tOAuthMcpKeys, } from './db.js';
...
// ── Discovery metadata ───────────────────────────────────────────────────────
export function handleAuthorizationServerMetadata(env) {
    return jsonResponse({
        issuer: env.MCP_URL,
...
export async function handleRegister(request, env) {
    ...
    if (!Array.isArray(redirectUris) || redirectUris.length === 0 || !redirectUris.every((u) => typeof u === 'string')) {
        return jsonResponse({ error: 'invalid_client_metadata', error_description: 'redirect_uris must be a non-empty array of strings' }, 400);
    }
    ...
    for (const uri of redirectUris) {
        try {
            ...
            if (parsed.protocol !== 'https:' && parsed.hostname !== 'localhost' && parsed.hostname !== '127.0.0.1') {
                return jsonResponse({ error: 'invalid_client_metadata', error_description: `redirect_uri must use HTTPS: ${uri}` }, 400);
            }
        }
        catch {
            return jsonResponse({ error: 'invalid_client_metadata', error_description: `Invalid redirect_uri: ${uri}` }, 400);
        }
    }
...
async function handleAuthorizationCodeGrant(params, db) {
    const code = params.get('code');
    const clientId = params.get('client_id');
    const codeVerifier = params.get('code_verifier');
    const redirectUri = params.get('redirect_uri');
    if (!code || !clientId || !codeVerifier) {
        return jsonResponse({ error: 'invalid_request', error_description: 'Missing code, client_id, or code_verifier' }, 400);
    }
    // Authenticate client
    ...
    const codes = await db
        ...
        .where(and(eq(tOAuthAuthorizationCodes.codeHash, codeHash), eq(tOAuthAuthorizationCodes.clientId, clientId), gt(tOAuthAuthorizationCodes.expiresAt, now)))
        .limit(1);
    if (codes.length === 0) {
...
async function handleRefreshTokenGrant(params, db) {
    ...
    const tokens = await db
        ...
        .where(and(eq(tOAuthRefreshTokens.tokenHash, tokenHash), eq(tOAuthRefreshTokens.clientId, client.clientId), gt(tOAuthRefreshTokens.expiresAt, now), isNull(tOAuthRefreshTokens.revokedAt)))
        .limit(1);
    if (tokens.length === 0) {
...
async function authenticateClient(params, db) {
    const clientId = params.get('client_id');
    if (!clientId)
        return null;
...
async function issueTokens(db, clientId, userId, scope) {
    const now = new Date();
...
export async function resolveOAuthToken(token, env) {
    ...
    const tokens = await db
        ...
        .from(tOAuthAccessTokens)
        .where(and(eq(tOAuthAccessTokens.tokenHash, tokenHash), gt(tOAuthAccessTokens.expiresAt, now), isNull(tOAuthAccessTokens.revokedAt)))
        .limit(1);
    if (tokens.length === 0)
        return null;
