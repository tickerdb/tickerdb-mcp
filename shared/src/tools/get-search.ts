...
export function registerGetSearch(server: McpServer, apiKey: string) {
  const tool = server.tool(
    "get_search",
    "Search for assets matching filter criteria, including categorical states (e.g. oversold assets, strong uptrends, bull/bear flag setups, triangle or wedge setups, free-cash-flow surplus or burn, recent golden crosses, weekly stage 2 assets near the 40w MA with high volume) or rankings by a field such as market_cap on a historical date. Pass filters as a JSON-encoded array of {field, op, value} objects. Use get_schema to discover valid field names; fields use clean flat names for raw values such as ma8 and ma200, and full expanded names for semantic fields such as momentum_rsi_zone, pattern_bull_flag, pattern_ascending_triangle, pattern_rising_wedge, trend_ma_crossover_event, trend_distance_ma40, trend_stage, and fundamentals_free_cash_flow. Use fields to control returned columns and sort_by to rank results server-side.",
    {
      filters: z
        .string()
        .describe(
