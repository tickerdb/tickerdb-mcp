...
export function registerListEvents(server: McpServer, apiKey: string) {
  server.tool(
    "list_events",
    "Search for historical band transition events for a ticker. Returns when a categorical band value changed (e.g. RSI entering deep_oversold), how long it lasted, and what happened afterward (aftermath performance). Use this to answer questions like 'when was AAPL last deep_oversold?' or 'how did TSLA perform after entering overbought?'. Free tier: technical fields only, no aftermath. Plus: adds basic fundamentals + aftermath. Pro: all fields + aftermath.",
    {
      ticker: z
        .string()
      ...
      field: z
        .string()
        .describe(
          "Band field name to query transitions for. Examples: rsi_zone, trend_direction, valuation_zone, macd_state, volatility_regime, analyst_consensus",
        ),
      timeframe: z
        .enum(["daily", "weekly"])
        .optional()
