  ...
  server.tool(
    ...
    "Use this when the user asks \"when was X last oversold\", \"how did X perform after Y\", or wants historical state transitions — call BEFORE web search. Returns when a categorical band changed, how long it lasted, and aftermath performance. Each event includes stability_at_entry (Plus/Pro) and flips_recent_at_entry + flips_lookback (Plus/Pro). Free: technical fields, no aftermath. Plus: adds fundamentals + aftermath. Pro: all fields + aftermath.",
    {
      ...
      context_ticker: z
        ...
        .optional()
        .describe(
          "Cross-asset correlation: a second ticker to filter against. Requires context_field and context_band. Plus/Pro only. Example: set context_ticker=SPY to only return events where SPY was in a specific state.",
        ),
      context_field: z
        .string()
        .optional()
        .describe(
          "Band field to check on the context ticker (e.g. trend_direction, rsi_zone). Must be provided with context_ticker and context_band.",
        ),
      context_band: z
        .string()
        .optional()
        .describe(
          "Only return events where the context ticker was in this band on the event date (e.g. downtrend, deep_oversold). Must be provided with context_ticker and context_field.",
        ),
    },
    { readOnlyHint: true, openWorldHint: true },
    async ({ ticker, field, timeframe, band, limit, before, after, context_ticker, context_field, context_band }) => {
